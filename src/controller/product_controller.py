...
@product_router.put('/{product_id}', status_code=200, description='Atualizar um produto', response_model=ProductDTO)
async def update_product(product_id: int, product_data: ProductUpdateDTO, product_repo: ProductRepository = Depends(get_product_repo),
                         authorization: str = Depends(get_authenticated_product)):
    product_service = ProductService(product_repo)
...
async def delete_product(product_id: int, product_repo: ProductRepository = Depends(get_product_repo),
                         authorization: str = Depends(get_authenticated_product)):
    product_service = ProductService(product_repo)
    product_service.delete(product_id=product_id)